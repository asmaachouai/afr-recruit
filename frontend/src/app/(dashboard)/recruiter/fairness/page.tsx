...
export default function FairnessPage() {
  ...
  return (
    <>
      <Header
        title="Rapports d'equite"
        description="Detection de biais et analyse d'impact disparate"
      />
      <div style={{ padding: "24px", background: MKC.surface, minHeight: "calc(100vh - 52px)" }}>
        ...
        <div style={{
          ...
          borderLeft: `4px solid ${MKC.rust}`, borderRadius: "8px",
          padding: "14px 16px", marginBottom: "20px",
        }}>
          <p style={{ fontSize: "12px", fontWeight: 700, color: MKC.dark, margin: "0 0 4px" }}>
            Regle des 4/5 (standard EEOC)
          </p>
          <p style={{ fontSize: "12px", color: MKC.muted, margin: 0, lineHeight: 1.6 }}>
            Si le taux de selection d'un groupe linguistique est inferieur a 80% du groupe le mieux classe,
            un impact disparate est detecte. Cela aide a identifier les biais involontaires
            contre les candidats francophones ou arabophones.
          </p>
        </div>

        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {[1, 2].map(i => <div key={i} style={{ height: "100px", background: MKC.warm, borderRadius: "10px" }} />)}
          </div>
        ) : jobs.length === 0 ? (
          <div style={{ background: "#fff", border: `1px solid ${MKC.border}`, borderRadius: "12px", padding: "48px", textAlign: "center" }}>
            <p style={{ fontSize: "13px", color: MKC.muted }}>Aucune offre a analyser</p>
          </div>
        ) : (
              ...
              return (
                <div key={job.id} style={{ background: "#fff", border: `1px solid ${MKC.border}`, borderRadius: "12px", overflow: "hidden" }}>
                  <div style={{
                    ...
                    <div>
                      <p style={{ fontSize: "14px", fontWeight: 700, color: MKC.dark, margin: "0 0 6px" }}>
                        {job.title}
                      </p>
                      {report && (
                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                          <span style={{
                            ...
                            background: report.overall_fair ? MKC.greenBg : MKC.amberBg,
                            color: report.overall_fair ? MKC.green : MKC.amber,
                          }}>
                            {report.overall_fair ? "Equitable" : "Biais detecte"}
                          </span>
                          <span style={{ fontSize: "12px", color: MKC.muted }}>
                            Ratio DI: {report.disparate_impact_ratio.toFixed(2)}
                          </span>
                        </div>
                      )}
                    </div>
                    <button
                      ...
                      style={{
                        ...
                        background: MKC.warm, color: MKC.rust,
                        border: `1px solid ${MKC.border}`,
                        borderRadius: "8px", fontSize: "12px", fontWeight: 600,
                        cursor: analyzing === job.id ? "not-allowed" : "pointer",
                      }}
                    >
                    ...
                    <div style={{ borderTop: `1px solid ${MKC.border}`, padding: "14px 16px" }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "10px", marginBottom: "12px" }}>
                        {[
                          ...
                          <div key={m.label} style={{ background: MKC.surface, borderRadius: "8px", padding: "10px 12px" }}>
                            <p style={{ fontSize: "10px", color: MKC.muted, margin: "0 0 3px" }}>{m.label}</p>
                            <p style={{ fontSize: "16px", fontWeight: 700, margin: 0, color: m.warn ? MKC.amber : MKC.dark }}>
                              {m.value}
                            </p>
                          </div>
                        ))}
                      ...
                      {Object.keys(report.language_distribution).length > 0 && (
                        <div style={{ marginBottom: "10px" }}>
                          <p style={{ fontSize: "11px", color: MKC.muted, margin: "0 0 6px" }}>
                      ...
                      {report.recommendations.slice(0, 2).map((rec, i) => (
                        <p key={i} style={{ fontSize: "12px", color: MKC.muted, margin: "4px 0", display: "flex", gap: "6px", lineHeight: 1.5 }}>
                          <span style={{ flexShrink: 0, color: MKC.rust }}>•</span>{rec}
                        </p>
                      ))}
                    </div>
                  )}

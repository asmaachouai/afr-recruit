...
export default function CVPage() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await api.post("/candidates/cv/upload", form, {
  ...
  return (
    <>
      ...
      <div style={{ padding: "24px", background: MKC.surface, minHeight: "calc(100vh - 52px)" }}>
        ...
        {/* Upload zone */}
        <div style={{
          background: "#fff", border: `2px dashed ${MKC.border}`,
          borderRadius: "12px", padding: "32px", textAlign: "center", marginBottom: "20px",
        }}>
          <div style={{
            width: "44px", height: "44px", borderRadius: "12px",
            background: MKC.warm, margin: "0 auto 12px",
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <svg width="20" height="20" fill="none" stroke="#C1440E" strokeWidth="2" viewBox="0 0 24 24">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12"/>
            </svg>
          </div>
          <p style={{ fontSize: "14px", fontWeight: 600, color: MKC.dark, margin: "0 0 4px" }}>
            Deposez votre CV
          </p>
          <p style={{ fontSize: "12px", color: MKC.muted, margin: "0 0 16px" }}>
            PDF, DOCX ou TXT — max 10 Mo
          </p>
          <input
            ref={fileRef} type="file" accept=".pdf,.docx,.txt"
            onChange={handleUpload} style={{ display: "none" }} id="cv-upload"
          />
          <label htmlFor="cv-upload">
            <span style={{
              display: "inline-block",
              padding: "8px 20px", background: uploading ? "#D4622A" : MKC.rust,
              color: MKC.cream, borderRadius: "8px",
          ...
          {uploadError && (
            <p style={{ fontSize: "12px", color: MKC.rust, marginTop: "10px" }}>
              {uploadError}
            </p>
          )}
        </div>
        ...
        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {[1, 2].map(i => (
              <div key={i} style={{ height: "72px", background: MKC.warm, borderRadius: "10px" }} />
            ))}
          </div>
        ) : cvs.length === 0 ? (
          <p style={{ textAlign: "center", color: MKC.muted, fontSize: "13px", padding: "40px 0" }}>
            Aucun CV depose pour le moment
          </p>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {cvs.map(cv => (
              <div key={cv.id} style={{
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <div style={{
                      ...
                      background: MKC.warm, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                    }}>
                    ...
                    {cv.ats_score != null && (
                      <div style={{ textAlign: "right" }}>
                        <p style={{ fontSize: "14px", fontWeight: 700, color: MKC.rust, margin: "0 0 1px" }}>
                          {cv.ats_score.toFixed(0)}/100
                        </p>
                        <p style={{ fontSize: "10px", color: MKC.muted, margin: 0 }}>
                          Grade {cv.ats_feedback?.grade}
                        </p>
                      </div>
                    )}
                    <svg width="14" height="14" fill="none" stroke={MKC.muted} strokeWidth="2" viewBox="0 0 24 24"
                ...
                {expanded === cv.id && cv.parsed_data && (
                  <div style={{ padding: "0 16px 16px", borderTop: `1px solid ${MKC.border}`, paddingTop: "14px" }}>
                    {cv.parsed_data.skills?.length > 0 && (
                      <div style={{ marginBottom: "14px" }}>
                        <p style={{ fontSize: "11px", fontWeight: 600, color: MKC.muted, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 8px" }}>
                    ...
                    {cv.ats_feedback && (
                      <div>
                        ...
                        {cv.ats_feedback.suggestions.slice(0, 3).map((s, i) => (
                          <p key={i} style={{ fontSize: "12px", color: MKC.rust, margin: "4px 0", display: "flex", gap: "6px" }}>
